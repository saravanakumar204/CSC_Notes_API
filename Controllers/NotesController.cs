...
namespace CSC_NotesService.Controllers
{
    ...
    public class NotesController : ControllerBase
    {
        ...
        public async Task<IActionResult> DeleteNote(int notesId)
        {
            try
            {
                var delete = await _notesService.DeleteNote(notesId);
                return Ok(delete);
            }
            catch (Exception ex)
            {
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<IActionResult> GetNotesById(int notesId)
        {
            try
            {
                var notes = await _notesService.GetNotesById(notesId);
                if (notes == null)
                    return NotFound("Not Found");
                return Ok(notes);
            }
